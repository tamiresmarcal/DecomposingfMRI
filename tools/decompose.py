...
"""Stage 4 -- fit a latent decomposition on some cohorts, project it onto others.
    ...
    python tools/decompose.py --atlas harvardoxford --window-s 30 60 120 300
...
WHAT IT IS NOT
--------------
Not a `fmri-decomp` subcommand. It reads no cohort YAML, it takes cohorts as
plain names, and it has no tests -- so it lives in tools/ next to the other
scripts rather than in the package. When the grid stops moving it should become
`fmri-decomp decompose` with the config carrying `train:`/`project:`, and the
window grid coming from `windows.sizes_s` instead of the command line.
"""
...
def read_cohort(root: Path, atlas: str, window_s, cohort: str, edges: list[str],
    ...
    ident_cols = [c for c in IDENT if c not in ("cohort", "task", "sub")]
    idents, blocks = [], []
    for p in paths:
...
def latents_for(ident: pd.DataFrame, X: np.ndarray, models: dict) -> pd.DataFrame:
...
def run_one(root: Path, window_s, args) -> None:
    ...
    models = fit_models(X_train, edges, args, meta={
        "atlas": atlas, "window_s": window_s, "train_cohorts": args.train,
        "project_cohorts": args.project, "n_train_rows": int(n_train),
        "seed": args.seed})
    del X_train
    gc.collect()

    models_dir.mkdir(parents=True, exist_ok=True)
    log(f"models -> {dump_models(models, stem).relative_to(root)}")

    # Second pass: one cohort in memory at a time.
    for cohort in cohorts:
        ident, X = read_cohort(root, atlas, window_s, cohort, edges)
        ident, X = drop_nan_rows(ident, X)
        lat = latents_for(ident, X, models)
        del ident, X
        gc.collect()
        path = out_dir / f"cohort={cohort}" / "data.parquet"
        path.parent.mkdir(parents=True, exist_ok=True)
        tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}")
        lat.to_parquet(tmp, index=False)
        os.replace(tmp, path)
        role = "train" if cohort in args.train else "projected"
        log(f"  {cohort:<14} {role:<10} {len(lat):>9,} rows x {lat.shape[1]} cols "
